"""Shared identities, filenames, and validation for professor-facing artifacts."""
...
_SPECS = {
    "dp_cnn": ProfessorModelSpec(
        "dp_cnn",
        "DP-CNN",
        "local-dp_cnn-recovered-v3-seed0",
    ),
    "dp_transformer": ProfessorModelSpec(
        "dp_transformer",
        "DP-Transformer",
        "local-dp_transformer-seed0",
    ),
    "ibc": ProfessorModelSpec("ibc", "IBC", "local-ibc-seed0"),
    "lstm_gmm": ProfessorModelSpec(
        "lstm_gmm",
        "LSTM-GMM",
...
def get_model_spec(model: str) -> ProfessorModelSpec:
    """Return one of the four frozen professor-facing model identities."""
    try:
        return _SPECS[model]
    except KeyError as error:
        raise ValueError(f"unsupported professor model: {model}") from error
